...
async function handleContactRequest(request) {
  ...
  if (!name || !phone) {
    return json({ error: "Name and phone or WhatsApp number are required." }, 400);
  }
  ...
  if (!emailResult.ok) {
    return json({
      ...
      message: emailResult.skipped
        ? "Demo request saved. Add email settings in Vercel to receive email alerts."
        : "Demo request saved, but email delivery failed. Check email settings in Vercel."
    }, 201);
  }

  return json({ ok: true, ticketId: String(ticket._id), emailSent: true, message: "Demo request sent. We will contact you soon." }, 201);
}
