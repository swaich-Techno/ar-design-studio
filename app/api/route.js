...
async function handlePublicBusiness(businessSlug) {
  ...
  const products = await Product.find({ businessId: business._id, isAvailable: true, publicationStatus: "PUBLISHED", stockStatus: { $ne: "OUT_OF_STOCK" } })
...
async function handlePublicProduct(businessSlug, productSlug) {
  ...
  const product = await Product.findOne({ businessId: business._id, slug: productSlug, isAvailable: true, publicationStatus: "PUBLISHED", stockStatus: { $ne: "OUT_OF_STOCK" } })
    .select("name slug category price discountPrice description shortDescription imageUrl galleryImages modelGlbUrl modelUsdzUrl offerText tags isAvailable stockStatus stockQuantity showStockToCustomers")
    .lean();
