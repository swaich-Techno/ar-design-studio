...
export default async function PublicCampaignPage({ params }) {
  const { businessSlug, campaignSlug } = await params;
  await connectDB();
  ...
  return (
    <main className="min-h-screen bg-surface">
      ...
      <section className="relative overflow-hidden bg-ink px-5 py-10 text-white">
        ...
        {campaign.bannerImageUrl ? <img src={campaign.bannerImageUrl} alt={`${campaign.title} QR campaign banner`} className="absolute inset-0 h-full w-full object-cover opacity-30" /> : null}
        <div className="relative mx-auto max-w-6xl">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-teal-200">{businessName}</p>
