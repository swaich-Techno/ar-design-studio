...
export function ProductForm({ productId }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setError(err.message || "Product save failed. Check your connection and try again.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <form onSubmit={submit} className="grid gap-5 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="grid gap-2 rounded-[22px] border border-slate-200 bg-slate-50 p-4">
        ...
        <ImageUploadField
          ...
          helperText="Best for web and Android AR. Keep under 100 MB; optimized GLB files load faster."
        />
        <ImageUploadField
          ...
          folder="ar-design-studio-models"
          accept=".usdz,model/vnd.usdz+zip,model/vnd.usd+zip,application/octet-stream"
          uploadType="model"
          extraFields={{ productId }}
          placeholder="https://.../product.usdz"
          helperText="Best for iPhone/iPad AR Quick Look. Paste a hosted URL if the file is already online."
        />
