...
export function StaffManager() {
  ...
  const [form, setForm] = useState({ name: "", email: "", password: "", permissions: { canEditProducts: false, canViewLeads: true, canDownloadQR: true, canEditAvailability: true } });
  ...
  async function submit(event) {
    ...
    setForm({ name: "", email: "", password: "", permissions: { canEditProducts: false, canViewLeads: true, canDownloadQR: true, canEditAvailability: true } });
    load();
  }
    ...
    <div className="grid gap-6">
      <form onSubmit={submit} className="grid gap-4 rounded-[26px] border border-slate-200 bg-white p-5">
        <div className="grid gap-4 md:grid-cols-3">
          <input className="field" placeholder="Staff name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required />
          <input className="field" type="email" placeholder="Staff email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required />
        ...
        <Button type="submit" variant="accent">Create staff account</Button>
      </form>
