...
export function ProfileForm() {
  ...
  return (
    <form onSubmit={submit} className="grid gap-5 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="grid gap-4 rounded-[22px] border border-slate-200 bg-slate-50 p-4 md:grid-cols-2">
        <input className="field" placeholder="Brand color, e.g. #0f766e" value={business.brandColor || ""} onChange={(e) => update("brandColor", e.target.value)} />
        <select className="field" value={business.catalogueTemplate || "MODERN"} onChange={(e) => update("catalogueTemplate", e.target.value)}>
          <option value="MODERN">Modern catalogue</option>
          <option value="RESTAURANT">Restaurant menu</option>
          <option value="RETAIL">Retail catalogue</option>
          <option value="LUXURY">Luxury / jewellery</option>
          <option value="REAL_ESTATE">Real estate</option>
        </select>
        <input className="field" placeholder="Google Analytics ID" value={business.googleAnalyticsId || ""} onChange={(e) => update("googleAnalyticsId", e.target.value)} />
        <input className="field" placeholder="Meta Pixel ID" value={business.metaPixelId || ""} onChange={(e) => update("metaPixelId", e.target.value)} />
      </div>
