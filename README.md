...
- Public free-demo requests saved into a Super Admin approval queue
- One-click approval for a 14-day, one-product demo workspace with one AR/3D slot
...
- Customer-facing AR button that opens the best available live 3D/AR experience and tracks AR intent
- Client-scoped Cloudinary upload folders for product images and model files
...
NEXT_PUBLIC_DEMO_WHATSAPP_NUMBER=917355008986
```
...
The public demo request form posts to `/api/contact` so requests are saved for Super Admin review. It also keeps a WhatsApp fallback link. Set `NEXT_PUBLIC_DEMO_WHATSAPP_NUMBER=917355008986` in Vercel to send direct demo chats to +91 73550 08986. Super Admin can review saved requests at `/super-admin/demo-requests` and approve a one-product trial workspace for a client.

For email delivery, add:

```env
CONTACT_TO_EMAIL=connect@bsocio.in
CONTACT_FROM_EMAIL=AR Design Studio <noreply@bsocio.in>
RESEND_API_KEY=
```
...
## Free Demo Approval Flow

1. A customer submits `/contact` with business name, contact person, email, WhatsApp number, industry, and requirement.
2. The backend saves a `SupportTicket` with `ticketType=DEMO_REQUEST`, sends email when Resend is configured, and shows clear success/failure state to the customer.
3. Super Admin opens `/super-admin/demo-requests`.
4. `Approve 1-product demo` creates or updates a trial `Business` with:
   - `productLimit=1`
   - `arProductLimit=1`
   - `storageLimitMB=250`
   - `subscriptionStatus=TRIAL`
   - `demoExpiresAt` set 14 days after approval
5. If no owner login exists for that email, the approval response shows a temporary password once. Share it with the client and replace it later through your normal support process.

## Storage Backend Recommendation

The current implementation still uses Cloudinary, but uploads now land under client-specific folders such as:

```txt
ar-design-studio/clients/client-slug/images/ar-design-studio-products
ar-design-studio/clients/client-slug/models/ar-design-studio-models
```

For a stronger inbuilt system later, build a small asset service instead of storing raw files directly in the Next.js app. Recommended design:

- `Asset` model: `businessId`, `productId`, `folder`, `assetType`, `provider`, `url`, `publicId`, `sizeMB`, `status`, `uploadedBy`.
- Private API routes for signed upload, asset listing, delete/replace, and per-client quota checks.
- Provider adapter layer so Cloudinary, S3/R2, or a personal storage backend can be swapped without changing product forms.
- Background optimization queue for image compression, GLB validation, USDZ pairing, thumbnail generation, and model-size warnings.
- Keep original files private when possible and serve optimized public versions to keep QR pages fast.
